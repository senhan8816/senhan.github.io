...
</code></pre><div class="line-numbers" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br></div></div><p>当我们配置完毕 pages 并保存代码之后,开发工具会自动帮我们在项目中创建 pages 目录,并创建 index 和 logs 两个页面（默认没有.wxss 和.json 文件，如果需要可以手动创建）。 并且默认展示 index 页面。调试器也不再提示错误
...
<ul>
<li>
<p>list 在 tabBar 中必须配置 list. 用于配置 tab 列表. 其中 list 接受一个数组，只能配置最少 2 个、最多 5 个 tab。tab 按数组的顺序排序，每个项都是一个对象，其属性值如下：</p>
<ul>
